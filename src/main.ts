...
const LEVEL_OBJECTIVE_TARGETS = [0, 38, 105, 210, 340, 500];
...
class InputState {
  ...
  constructor() {
    const set = (e: KeyboardEvent, value: boolean): void => {
      ...
      if (k === 'shift' || c === 'ShiftLeft' || c === 'ShiftRight') this.boost = value;
    };
...
class Game {
  ...
  private objectiveRateDisplay = 0;
  ...
  private updateDifficulty(dt: number): boolean {
    ...
    if (this.level === 5 && this.levelObjective >= targetForLevel) {
  ...
  private updateShip(dt: number): void {
    ...
    const boost = this.input.boost && this.fuel > 0 && mag > 0.0001;
    ...
    if (mag > 0.0001 && this.fuel > 0) {
      const thrustPower = boost ? 1.9 : 0.9;
      thrustX = (ax / mag) * thrustPower;
      thrustY = (ay / mag) * thrustPower;
      this.shipHeading = Math.atan2(thrustY, thrustX) - Math.PI / 2;

      const baseBurn = (boost ? 14.2 : 4.9) * (1 + preDanger * 0.95);
      this.fuel = Math.max(0, this.fuel - baseBurn * dt);
    }
    const thrustTarget = mag > 0.0001 && this.fuel > 0 ? (boost ? 1 : 0.68) : 0;
    this.thrustVisual = lerp(this.thrustVisual, thrustTarget, Math.min(1, dt * 12));
    this.boostVisual = lerp(this.boostVisual, boost ? 1 : 0, Math.min(1, dt * 14));
    ...
    const radiationDrain = radiationPulse * (this.level >= 4 ? 1.6 : 0.8);

    const gravityAmp = 1.0 + preDanger * 1.4 + preDanger * preDanger * 1.8;
    const thrustAuthority = 0.82 + preDanger * 0.28;
    this.shipVel.x += (thrustX * thrustAuthority + (gravA.x + gravB.x) * gravityAmp) * dt;
    this.shipVel.y += (thrustY * thrustAuthority + (gravA.y + gravB.y) * gravityAmp) * dt;
    ...
    const survivalBase = this.level === 1 ? 0.24 : 0.12;
    const controlBonus = mag > 0.0001 ? 0.16 : 0.0;
    const sustainedRiskBonus = danger > 0.18 && danger < 0.9 ? 0.42 : 0.0;
    const objectivePerSecond =
      ...
      binaryBonus +
      sustainedRiskBonus;
    this.objectiveRateDisplay = lerp(this.objectiveRateDisplay, objectivePerSecond, Math.min(1, dt * 6));
    this.levelObjective += objectivePerSecond * dt;

    // Risk reward: controlled skimming near ISCO can slowly reclaim fuel.
    const skimBand = danger > 0.26 && danger < 0.88;
    if (skimBand && !boost && this.fuel < 100) {
      const skimGain = (danger - 0.2) * 0.55;
      this.fuel = Math.min(100, this.fuel + skimGain * dt);
    }
  ...
  private uploadUniforms(dt: number): void {
    ...
    const ship = new Float32Array(12);
    ...
    ship.set([this.thrustVisual, this.boostVisual, 0, 0], 8);
    this.device.queue.writeBuffer(this.shipUniformBuffer, 0, ship);

    const post = new Float32Array(12);
    post.set([this.bhA.x, this.bhA.y, this.bhA.mass * 3.0, this.elapsed], 0);
    post.set([this.bhB.x, this.bhB.y, this.bhB.mass * 2.7, this.elapsed], 4);
  ...
  private drawHud(): void {
    ...
    this.hud.textContent = [
      ...
      `Objective Rate: ${this.objectiveRateDisplay.toFixed(2)} /s`,
      `Boost: ${this.input.boost ? 'ON' : 'OFF'}`,
